            ...
            async with engine.connect() as connection:
                return await connection.run_sync(
                    lambda sync: {
                        ...
                        "run_indexes": {
                            item["name"]: item
                            for item in inspect(sync).get_indexes("daily_candidate_runs")
                        },
                        "candidate_uniques": {
                            item["name"]: item
                            for item in inspect(sync).get_unique_constraints("daily_candidates")
                        },
                        "decision_fks": {
                            item["name"]: item
                            for item in inspect(sync).get_foreign_keys("editorial_decisions")
                        },
                    }
                )
        finally:
...
def test_m5b_migration_creates_candidate_and_decision_snapshot_schema() -> None:
    ...
    schema = _schema()
    assert {"daily_candidate_runs", "daily_candidates", "editorial_decisions"} <= schema["tables"]
    assert schema["run_indexes"]["uq_daily_candidate_runs_success_input"]["unique"]
    assert {"uq_daily_candidates_run_event", "uq_daily_candidates_run_rank"} <= set(
        schema["candidate_uniques"]
    )
