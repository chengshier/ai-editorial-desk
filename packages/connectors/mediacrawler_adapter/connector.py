...
from packages.connectors.base import CollectionItemError, CollectionResult, CollectRequest
...
class MediaCrawlerConnector:
    ...
    async def collect(self, request: CollectRequest) -> CollectionResult:
        ...
        if mode.value not in spec.allowed_modes:
            raise ConnectorCapabilityError(
                f"{platform.value} does not support MediaCrawler mode {mode.value} in M2-B"
            )
        ...
        if mode is MediaCrawlerMode.SEARCH:
            keyword = request.query or self._string_value(parameters.get("keyword"))
            keyword = keyword or self._first_string(parameters.get("keywords"))
            if keyword is None:
            ...
            content_ids = request.target_ids or self._string_tuple(parameters.get("content_ids"))
            content_ids = content_ids or self._string_tuple(parameters.get("target_ids"))
    ...
    def _to_collection_result(self, envelope: MediaCrawlerResultEnvelope) -> CollectionResult:
            ...
            try:
                ...
                errors.append(
                    CollectionItemError(
                        ...
                        message="MediaCrawler item failed platform mapping",
                        external_ref=getattr(exc, "external_ref", None),
                    )
                )
        ...
        for comment in envelope.comments:
            try:
                ...
                errors.append(
                    CollectionItemError(
                        code="mediacrawler_comment_unmapped",
                        message="MediaCrawler comment failed platform mapping",
                        external_ref=getattr(exc, "external_ref", None),
                    )
                )
