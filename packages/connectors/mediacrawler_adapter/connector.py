...
from packages.connectors.base import BaseConnector, CollectionItemError, CollectionResult, CollectRequest
from packages.connectors.mediacrawler_adapter.adapter import MediaCrawlerAdapter
from packages.connectors.mediacrawler_adapter.errors import MediaCrawlerAdapterError, MediaCrawlerErrorCode
...
class MediaCrawlerConnector(BaseConnector):
    ...
    async def collect(self, request: CollectRequest) -> CollectionResult:
        if request.run_id is None or request.platform is None:
            raise MediaCrawlerAdapterError(MediaCrawlerErrorCode.RESULT_MALFORMED, "MediaCrawler collection requires Runtime run_id and platform context")
        try:
            ...
            raise MediaCrawlerAdapterError(MediaCrawlerErrorCode.RESULT_MALFORMED, "MediaCrawler collection request is outside the versioned protocol") from exc
        spec = get_platform_spec(platform.value)
        if mode.value not in spec.allowed_modes:
            raise ConnectorCapabilityError(f"{platform.value} does not support MediaCrawler mode {mode.value} in M2-B")
        parameters = dict(request.parameters)
        keyword: str | None = None
        creator_id: str | None = None
        content_ids: tuple[str, ...] = ()
        if mode is MediaCrawlerMode.SEARCH:
            keyword = request.query or self._string_value(parameters.get("keyword")) or self._first_string(parameters.get("keywords"))
            if keyword is None:
            ...
            content_ids = request.target_ids or self._string_tuple(parameters.get("content_ids")) or self._string_tuple(parameters.get("target_ids"))
        ...
        comment_limit = self._bounded_integer(parameters.get("comment_limit"), default=20 if include_comments else 0, minimum=0, maximum=50)
        if not include_comments:
            comment_limit = 0
            include_subcomments = False
        timeout_seconds = self._bounded_integer(parameters.get("timeout_seconds"), default=self.adapter.settings.mediacrawler_timeout_seconds, minimum=30, maximum=1800)
        envelope = await self.adapter.invoke(
            MediaCrawlerInvocation(
                run_id=run_id, platform=platform, mode=mode, source_id=source_id,
                keyword=keyword, creator_id=creator_id, content_ids=content_ids,
                requested_limit=request.limit, comment_limit=comment_limit,
                include_comments=include_comments, include_subcomments=include_subcomments,
                checkpoint=request.checkpoint, account_ref=request.account_ref or request.account_id,
                browser_profile_ref=request.browser_profile_ref, timeout_seconds=timeout_seconds,
            )
        )
        return self._to_collection_result(envelope, allow_comments=include_comments)

    def _to_collection_result(self, envelope: MediaCrawlerResultEnvelope, *, allow_comments: bool) -> CollectionResult:
        mapper = mediacrawler_mapper_registry.get(envelope.platform.value)
        signals = []
        comments = []
        errors = [CollectionItemError(error.code, error.message, error.external_ref) for error in envelope.errors]
        failed_items = 0
        failed_comments = 0
        for item in envelope.items:
            try:
                ...
                failed_items += 1
                errors.append(CollectionItemError("mediacrawler_item_unmapped", "MediaCrawler item failed platform mapping", getattr(exc, "external_ref", None)))
        if envelope.items and not signals:
            raise MediaCrawlerAdapterError(MediaCrawlerErrorCode.PARSE_ERROR, f"{envelope.platform.value} mapper could not recognize the result format")
        if envelope.comments and not allow_comments:
            failed_comments = len(envelope.comments)
            errors.append(CollectionItemError("mediacrawler_unexpected_comments", "MediaCrawler returned comments although comment collection was disabled", None))
        else:
            for comment in envelope.comments:
                try:
                    ...
                    failed_comments += 1
                    errors.append(CollectionItemError("mediacrawler_comment_unmapped", "MediaCrawler comment failed platform mapping", getattr(exc, "external_ref", None)))
        return CollectionResult(
            signals=tuple(signals), checkpoint=envelope.checkpoint, errors=tuple(errors[:100]),
            metadata={
                ...
                "mapped_count": len(signals), "failed_map_count": failed_items,
                "mapped_comment_count": len(comments), "failed_comment_map_count": failed_comments,
