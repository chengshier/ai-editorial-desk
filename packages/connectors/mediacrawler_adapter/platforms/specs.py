...
def _contains_mode(mode: str) -> dict[str, Any]:
    return {
        "properties": {"modes": {"contains": {"const": mode}}},
        "required": ["modes"],
    }
...
def build_config_schema(spec: MediaCrawlerPlatformSpec) -> dict[str, Any]:
    ...
    if spec.search:
        properties["keyword"] = {
            "type": "string", "title": "关键词", "minLength": 1, "maxLength": 200,
        }
    ...
    if spec.detail or "comments" in spec.allowed_modes:
        properties["content_ids"] = {
            "type": "array", "title": "内容 ID / URL", "minItems": 1,
            "maxItems": 100, "uniqueItems": True,
            "items": {"type": "string", "minLength": 1, "maxLength": 500},
        }
        for mode in (mode for mode in ("detail", "comments") if mode in spec.allowed_modes):
            all_of.append({"if": _contains_mode(mode), "then": {"required": ["content_ids"]}})
    if spec.creator:
        properties["creator_id"] = {
            "type": "string", "title": "创作者 ID / URL", "minLength": 1, "maxLength": 500,
        }
...
def build_ui_schema(spec: MediaCrawlerPlatformSpec) -> dict[str, Any]:
    ui: dict[str, Any] = {
        "modes": {
            "widget": "checkbox_group", "label": "采集模式", "order": 10,
            "help": "仅展示当前 pinned MediaCrawler 通过主系统安全边界可执行的模式。",
        },
        "include_comments": {
            "label": "采集一级评论", "order": 50,
        ...
        "include_subcomments": {
            "label": "采集二级评论", "order": 70,
            "visible_when": {"field": "include_comments", "equals": True},
            "help": "默认关闭。M2-C 不进行任何真实平台评论采集。",
        },
        "timeout_seconds": {"label": "运行超时（秒）", "order": 80},
    }
    if spec.search:
        ui["keyword"] = {
            "widget": "text", "label": "关键词", "order": 20,
            "visible_when": {"field": "modes", "contains": "search"},
        }
    if "content_ids" in build_config_schema(spec)["properties"]:
        ui["content_ids"] = {
            "widget": "tags", "label": "内容 ID / URL", "order": 30,
            "visible_when": {"field": "modes", "contains_any": ["detail", "comments"]},
        }
    if spec.creator:
        ui["creator_id"] = {
            "widget": "text", "label": "创作者 ID / URL", "order": 40,
