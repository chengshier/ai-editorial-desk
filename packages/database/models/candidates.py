...
class DailyCandidateRunRecord(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    """Immutable persisted snapshot of one deterministic daily candidate run."""

    __tablename__ = "daily_candidate_runs"
    __table_args__ = (
        CheckConstraint(
            "window_end_at > window_start_at",
            name="candidate_run_window_valid",
        ),
        CheckConstraint("requested_limit > 0", name="candidate_run_limit_positive"),
        CheckConstraint(
            "scanned_event_count >= 0",
            name="candidate_run_scanned_nonnegative",
        ),
        CheckConstraint(
            "eligible_event_count >= 0",
            name="candidate_run_eligible_nonnegative",
        ),
        CheckConstraint(
            "candidate_count >= 0",
            name="candidate_run_candidate_nonnegative",
        ),
        CheckConstraint(
            "skipped_event_count >= 0",
            name="candidate_run_skipped_nonnegative",
        ),
        CheckConstraint(
            "char_length(btrim(timezone)) > 0",
            name="candidate_run_timezone_nonempty",
        ),
        CheckConstraint(
            "char_length(btrim(ranking_version)) > 0",
            name="candidate_run_ranking_nonempty",
        ),
        CheckConstraint(
            "char_length(input_hash) = 64",
            name="candidate_run_input_hash_sha256",
        ),
        CheckConstraint(
            "char_length(btrim(actor)) > 0",
            name="candidate_run_actor_nonempty",
        ),
        Index(
    ...
    status: Mapped[CandidateRunStatus] = mapped_column(
        string_enum(CandidateRunStatus, name="candidate_run_status"),
        nullable=False,
    )
    ...
    skip_summary: Mapped[dict[str, Any]] = mapped_column(
        JSONB,
        nullable=False,
        default=dict,
    )
    mode: Mapped[CandidateRunMode] = mapped_column(
        string_enum(CandidateRunMode, name="candidate_run_mode"),
        nullable=False,
    )
...
class DailyCandidateRecord(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    """Immutable rank and editorial context snapshot within one candidate run."""
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="daily_candidate_unknown_nonnegative",
        ),
        CheckConstraint(
            "source_count_snapshot >= 0",
            name="daily_candidate_source_nonnegative",
        ),
        CheckConstraint(
            "platform_count_snapshot >= 0",
            name="daily_candidate_platform_nonnegative",
        ),
        CheckConstraint(
        ...
        Index(
            "ix_daily_candidates_run_group_rank",
            "run_id",
            "candidate_group",
            "rank",
        ),
    ...
    run_id: Mapped[UUID] = mapped_column(
        ForeignKey("daily_candidate_runs.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
    ...
    candidate_group: Mapped[CandidateGroup] = mapped_column(
        string_enum(CandidateGroup, name="candidate_group"),
        nullable=False,
    )
    ...
    event_status_snapshot: Mapped[EventStatus] = mapped_column(
        string_enum(EventStatus, name="candidate_event_status"),
        nullable=False,
    )
    event_last_updated_at_snapshot: Mapped[datetime] = mapped_column(
        UTCDateTime(),
        nullable=False,
    )
    ...
    effective_assessment_hash: Mapped[str] = mapped_column(
        String(64),
        nullable=False,
    )
    ...
    effective_risk_level: Mapped[EditorialRiskLevel] = mapped_column(
        string_enum(
            EditorialRiskLevel,
            name="candidate_effective_risk_level",
        ),
        nullable=False,
    )
    recommended_format: Mapped[EditorialRecommendedFormat] = mapped_column(
        string_enum(
            EditorialRecommendedFormat,
            name="candidate_recommended_format",
        ),
        nullable=False,
    )
    ...
    ranking_components: Mapped[dict[str, Any]] = mapped_column(
        JSONB,
        nullable=False,
        default=dict,
    )
    card_exists_snapshot: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
    )
    draft_exists_snapshot: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
    )
...
class EditorialDecisionRecord(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    """Append-only human decision, independent of lifecycle and algorithmic rank."""

    __tablename__ = "editorial_decisions"
    __table_args__ = (
        CheckConstraint(
            "char_length(btrim(actor)) > 0",
            name="editorial_decision_actor_nonempty",
        ),
        CheckConstraint(
            "char_length(btrim(reason)) > 0",
            name="editorial_decision_reason_nonempty",
        ),
        CheckConstraint(
            "candidate_context_hash IS NULL OR "
            "char_length(candidate_context_hash) = 64",
    ...
    candidate_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("daily_candidates.id", ondelete="RESTRICT"),
        index=True,
    )
    decision: Mapped[EditorialDecisionType] = mapped_column(
        string_enum(EditorialDecisionType, name="editorial_decision_type"),
        nullable=False,
    )
    previous_decision_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("editorial_decisions.id", ondelete="RESTRICT"),
        index=True,
    )
