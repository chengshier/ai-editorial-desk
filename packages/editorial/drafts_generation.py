                ...
                input_hash = stable_hash(
                    {
                        ...
                        "body": body,
                        "references": [
                            (str(ref.claim_id), ref.section_key, ref.usage.value)
    ...
    async def chain(
        ...
        async with self.session_factory() as session:
            ...
            if draft is None or draft.event_id != event_id:
                raise ResourceNotFoundError("Draft不存在")
            return tuple(
                await EditorialDraftRepository(session).list_chain(draft.draft_chain_id)
            )
...
def _validate_human_references(
    ...
    for ref in references:
        section_key = normalize_text(ref.section_key)
        ...
        marker = (section_key, ref.claim_id)
        if marker in seen:
            raise DraftValidationError("同一section不能重复引用同一Claim")
        seen.add(marker)
