...
export function EventCardPackSection({api,eventId,canWrite,onChanged}:{api:WorkbenchApi;eventId:string;canWrite:boolean;onChanged:()=>void}){
 const[cards,setCards]=useState<EventCard[]>([]);const[packs,setPacks]=useState<EditorialPack[]>([]);const[cardId,setCardId]=useState('');const[loading,setLoading]=useState(true);const[error,setError]=useState<string|null>(null);const[mutating,setMutating]=useState(false)
