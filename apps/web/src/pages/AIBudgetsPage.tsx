...
export function AIBudgetsPage({ api }: Props) {
  ...
  return <section className="panel">
    ...
    <div className="form-grid operations-form">
      <label className="field-sm">预算范围<select value={draft.scope_type} onChange={e => setDraft({ ...draft, scope_type: e.target.value, scope_key: e.target.value === 'global' ? 'global' : '' })}><option value="global">全局</option><option value="task">任务</option><option value="provider">服务商</option></select></label>
      <label className="field-md">范围标识<input value={draft.scope_key} onChange={e => setDraft({ ...draft, scope_key: e.target.value })} /></label>
      <label className="field-md">未知用量处理策略<select value={draft.unknown_usage_policy} onChange={e => setDraft({ ...draft, unknown_usage_policy: e.target.value })}><option value="block">阻止调用</option><option value="allow_once">允许一次</option></select></label>
      <label className="field-sm">每日成本上限<input value={draft.daily_cost_limit} onChange={e => setDraft({ ...draft, daily_cost_limit: e.target.value })} /></label>
      <label className="field-sm">每月成本上限<input value={draft.monthly_cost_limit} onChange={e => setDraft({ ...draft, monthly_cost_limit: e.target.value })} /></label>
      <label className="field-sm">每日 Token 上限<input value={draft.daily_token_limit} onChange={e => setDraft({ ...draft, daily_token_limit: e.target.value })} /></label>
    </div>
    <div className="actions"><button className="primary" onClick={() => void create()}>创建预算</button></div>
    {!error && (budgets.length===0?<Empty text="暂无 AI 预算" helper="创建预算后，会在这里显示成本与 Token 约束。"/>:<div className="table-wrap"><table><thead><tr><th>预算范围</th><th>每日成本</th><th>每月成本</th><th>每日 Token</th><th>未知用量策略</th><th>状态</th><th>操作</th></tr></thead><tbody>{budgets.map(budget => <tr key={budget.id}><td>{scopeLabel[budget.scope_type]||budget.scope_type} · {budget.scope_key}</td><td>{numberLabel(budget.daily_cost_limit)}</td><td>{numberLabel(budget.monthly_cost_limit)}</td><td>{budget.daily_token_limit ?? '—'}</td><td>{policyLabel[budget.unknown_usage_policy]||budget.unknown_usage_policy}</td><td>{enabledLabel(budget.enabled)}</td><td><div className="actions"><button onClick={() => void edit(budget)}>编辑</button><button onClick={() => void toggle(budget)}>{budget.enabled ? '停用' : '启用'}</button></div></td></tr>)}</tbody></table></div>)}
