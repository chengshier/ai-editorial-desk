...
type SafeDiagnostic = {
  ...
  cdp_connect_failed?: boolean
  profile_error?: boolean
...
const stageLabel: Record<string, string> = {
  ...
  client_create: '创建平台客户端',
  login_state: '检查登录状态',
  search: '执行搜索采集',
}

function diagnosticFor(run: Run): SafeDiagnostic | null {
  const value = run.metadata?.subprocess_diagnostic
  return value && typeof value === 'object' && !Array.isArray(value) ? value as SafeDiagnostic : null
}

function diagnosticSuggestion(diagnostic: SafeDiagnostic): string {
  if (diagnostic.auth_required) return '检查当前运行选择的平台账号与浏览器登录态；不要通过换号或绕过平台验证重试。'
  if (diagnostic.cdp_connect_failed) return '确认浏览器调试端点已启动，并检查当前账号关联的浏览器环境是否可连接。'
  if (diagnostic.profile_error) return '检查平台账号关联的浏览器 Profile 是否存在并可被当前后端进程访问。'
  if (diagnostic.dependency_error) return `检查 MediaCrawler 运行环境依赖${diagnostic.dependency_module ? `：${diagnostic.dependency_module}` : ''}。`
  if (diagnostic.configuration_error) return '检查连接器实例、信源目标与 MediaCrawler 运行配置是否完整。'
  if (diagnostic.timeout) return '检查网络与运行超时设置；确认不是平台风险后再决定是否人工重试。'
  return '先查看下方技术详情确认运行阶段；若没有更具体的安全诊断，再检查后端运行日志。'
}
...
export function RunsPage({ api }: { api: AdminApi }) {
  ...
  return <Panel title="运行记录" actions={<><select aria-label="运行状态" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">全部状态</option>{['pending', 'running', 'succeeded', 'partial', 'failed', 'cancelled', 'paused_risk'].map((item) => <option key={item} value={item}>{runStatusLabel[item]}</option>)}</select><button onClick={load}>刷新</button></>}>
    <div className="page-intro"><p>查看采集任务的执行结果、数量统计与风险状态。失败时优先展示经过脱敏的可操作诊断，原始检查点与预算数据收纳在技术详情中。</p><span className="readonly-note">历史 / 审计</span></div>
