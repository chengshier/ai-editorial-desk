...
it('saves task-level max output tokens without replacing unrelated route config', async () => {
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes('/api/v1/admin/ai/routes')) {
      return Response.json({ items: [route], page: 1, page_size: 100, total: 1, has_next: false })
    }
    if (url.includes('/api/v1/admin/ai/models')) {
...
it('shows code fallback and no token editor for unrelated tasks', async () => {
  const routes = [
    {
      id: 'route-evidence',
  ...
  vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => {
    const url = String(input)
    if (url.includes('/api/v1/admin/ai/routes')) {
      return Response.json({ items: routes, page: 1, page_size: 100, total: 2, has_next: false })
    }
    return Response.json({ items: [], page: 1, page_size: 100, total: 0, has_next: false })
  }))
  ...
  const evidenceInput = await screen.findByLabelText('evidence_extraction 最大输出 Token')
  expect(evidenceInput).toHaveAttribute('placeholder', '4096')
