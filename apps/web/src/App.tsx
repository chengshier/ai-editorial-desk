...
type PageKey=typeof groups[number]['items'][number][0]
...
export default function App(){
 ...
 const navigate=(key:string)=>{if(allPages.some(([k])=>k===key)){setEventId(null);setPage(key as PageKey)}};const openEvent=(id:string)=>{setEventId(id);setPage('events')}
 const title=eventId?'Event Workbench':allPages.find(([k])=>k===page)?.[1]
