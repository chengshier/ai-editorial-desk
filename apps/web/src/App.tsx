...
import { Activity, Blocks, BookmarkCheck, CalendarClock, CircleDot, Cpu, Gauge, History, LayoutDashboard, ListChecks, Menu, PlugZap, Route, Rss, ScrollText, Send, ShieldAlert, Sparkles, UserRound, WalletCards, type LucideIcon } from 'lucide-react'
...
import { CollectionBudgetsPage } from './pages/CollectionBudgetsPage'
...
type PageKey='overview'|'candidates'|'events'|'publications'|'performance'|'sources'|'collection-budgets'|'schedules'|'runs'|'checkpoints'|'risk'|'definitions'|'instances'|'ai-providers'|'ai-routes'|'ai-budgets'|'ai-invocations'
...
const groups:readonly NavGroup[]=[
 ...
 {label:'内容资源',items:[['sources','信源',Rss],['collection-budgets','采集预算',Gauge],['schedules','采集任务',CalendarClock],['runs','运行记录',History],['checkpoints','检查点',BookmarkCheck],['risk','平台账号 / 风险',ShieldAlert]]},
 {label:'系统配置',items:[['definitions','连接器能力',Blocks],['instances','连接器实例',PlugZap]]},
...
const pageDescriptions:Record<PageKey,string>={overview:'优先处理高风险、需要人工判断的编辑事项',candidates:'浏览候选快照并记录人工编辑判断',events:'核验事件上下文并推进内容生产',publications:'只记录真实已发布结果',performance:'读取已录入的发布表现反馈',sources:'配置具体采集目标并在真实执行前完成采集前检查','collection-budgets':'配置正式采集的单次量、每日量与并发边界',schedules:'安排和查看正式采集调度',runs:'检查采集执行、失败诊断与恢复入口',checkpoints:'查看采集进度与恢复点',risk:'配置平台账号并处理平台风险事件',definitions:'查看系统已注册的连接器能力与运行开关',instances:'管理连接器实例与公共运行配置','ai-providers':'管理 AI Provider 可用性','ai-routes':'管理 AI 路由、任务级生成上限与版本','ai-budgets':'查看 AI 成本与 Token 预算约束','ai-invocations':'审计 AI 调用元数据'}
...
export default function App(){
 ...
 return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark" aria-hidden="true"><Sparkles size={20}/></span><div><h1>AI 编辑部</h1><p>Editorial Intelligence</p></div></div><nav>{groups.map(group=><section className="nav-group" key={group.label}><span>{group.label}</span>{group.items.map(([key,label,Icon])=><button key={key} className={!eventId&&page===key?'active':''} onClick={()=>navigate(key)}><Icon className="nav-icon" size={18} strokeWidth={1.8}/><span>{label}</span></button>)}</section>)}</nav><details className="auth-box"><summary><span className="session-label"><UserRound size={16}/>会话与编辑身份</span><span className={`connection ${fullyConfigured?'ok':'warn'}`}>{fullyConfigured?'已就绪':'待配置'}</span></summary><div className="auth-fields"><label>API 地址<input value={draft.apiBaseUrl} onChange={e=>setDraft({...draft,apiBaseUrl:e.target.value})}/></label><label>管理员 Token<input type="password" autoComplete="off" value={draft.adminToken} onChange={e=>setDraft({...draft,adminToken:e.target.value})}/></label><label>执行者 ID<input value={draft.actorId} onChange={e=>setDraft({...draft,actorId:e.target.value})}/></label><div className="actions"><button className="primary" onClick={apply}>应用会话配置</button><button onClick={clear}>清除</button></div><small>Token 与执行者 ID 仅存于 sessionStorage；发布、效果反馈和人工判断写操作仍要求显式执行者。</small></div></details></aside><main className="main-shell"><header className="topbar"><div className="topbar-title"><Menu className="topbar-menu" size={21} aria-hidden="true"/><span><strong>{title}</strong><small>{description}</small></span></div><span className={`connection ${fullyConfigured?'ok':'warn'}`}>{fullyConfigured?'管理员与执行者已配置':tokenConfigured?'只读已就绪 · 请配置执行者 ID':'请配置管理员 Token 与执行者 ID'}</span></header><div className="content"><div className={`page-inner ${pageMode}`}>
  {eventId?<><EventWorkbenchPage api={workbench} workflowApi={workflow} eventId={eventId} actorConfigured={actorConfigured} onBack={()=>setEventId(null)} onOpenEvent={openEvent}/><EventPublicationSection api={publication} eventId={eventId} onNavigate={navigate}/></>:<>
   {page==='overview'&&<EditorialOverviewPage api={workbench} adminApi={adminApi} onNavigate={navigate}/>} {page==='candidates'&&<EditorialCandidatesPage api={workflow} publicationApi={publication} actorConfigured={actorConfigured} onOpenEvent={openEvent}/>} {page==='events'&&<EditorialEventsPage api={workbench} onOpenEvent={openEvent}/>} {page==='publications'&&<PublicationsPage api={publication} workbench={workbench} actorConfigured={actorConfigured} onOpenEvent={openEvent}/>} {page==='performance'&&<PerformancePage api={publication} actorConfigured={actorConfigured}/>} {page==='definitions'&&<DefinitionsPage api={adminApi}/>} {page==='instances'&&<InstancesPage api={adminApi} onNavigate={navigate}/>} {page==='sources'&&<SourcesPage api={adminApi} onNavigate={navigate}/>} {page==='collection-budgets'&&<CollectionBudgetsPage api={adminApi}/>} {page==='schedules'&&<SchedulesPage api={adminApi} onNavigate={navigate}/>} {page==='runs'&&<RunsPage api={adminApi} onNavigate={navigate}/>} {page==='checkpoints'&&<CheckpointsPage api={adminApi}/>} {page==='risk'&&<AccountsRiskPage api={adminApi}/>} {page==='ai-providers'&&<AIProvidersPage api={adminApi}/>} {page==='ai-routes'&&<AIRoutesPage api={adminApi}/>} {page==='ai-budgets'&&<AIBudgetsPage api={adminApi}/>} {page==='ai-invocations'&&<AIInvocationsPage api={adminApi}/>}
  </>}
 </div></div></main></div>
}
