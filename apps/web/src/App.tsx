...
export default function App(){
 ...
 return <div className="app-shell"><aside className="sidebar"><div><h1>AI 编辑部</h1><p>M5-C Publication / Performance Feedback</p></div><nav>{groups.map(group=><section className="nav-group" key={group.label}><span>{group.label}</span>{group.items.map(([key,label])=><button key={key} className={!eventId&&page===key?'active':''} onClick={()=>navigate(key)}>{label}</button>)}</section>)}</nav><div className="auth-box"><label>API Base<input value={draft.apiBaseUrl} onChange={e=>setDraft({...draft,apiBaseUrl:e.target.value})}/></label><label>Admin Token<input type="password" autoComplete="off" value={draft.adminToken} onChange={e=>setDraft({...draft,adminToken:e.target.value})}/></label><label>Actor ID<input value={draft.actorId} onChange={e=>setDraft({...draft,actorId:e.target.value})}/></label><div className="actions"><button onClick={apply}>应用会话配置</button><button onClick={clear}>清除</button></div><small>Token / Actor 仅存 sessionStorage。Publication、Performance、Human Decision 写操作继续要求显式 Actor。</small></div></aside><main><header className="topbar"><div><strong>{title}</strong><span className={`connection ${fullyConfigured?'ok':'warn'}`}>{fullyConfigured?'Admin + Actor 已配置':tokenConfigured?'Read ready · Configure Actor ID':'请配置 Admin Token / Actor'}</span></div><span>内部编辑工作台 · Publication 仅记录真实已发布结果</span></header><div className="content">
