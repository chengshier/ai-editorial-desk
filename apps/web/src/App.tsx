...
type PageKey='overview'|'candidates'|'events'|'sources'|'schedules'|'runs'|'checkpoints'|'risk'|'definitions'|'instances'|'ai-providers'|'ai-routes'|'ai-budgets'|'ai-invocations'
type NavItem=readonly[PageKey,string]
type NavGroup={label:string;items:readonly NavItem[]}
const groups:readonly NavGroup[]=[
 {label:'Editorial',items:[['overview','Overview'],['candidates','Candidates'],['events','Events']]},
...
export default function App(){
 ...
 return <div className="app-shell"><aside className="sidebar"><div><h1>AI 编辑部</h1><p>M5-B Daily Editorial Workflow</p></div><nav>{groups.map(group=><section className="nav-group" key={group.label}><span>{group.label}</span>{group.items.map(([key,label])=><button key={key} className={!eventId&&page===key?'active':''} onClick={()=>navigate(key)}>{label}</button>)}</section>)}</nav><div className="auth-box"><label>API Base<input value={draft.apiBaseUrl} onChange={e=>setDraft({...draft,apiBaseUrl:e.target.value})}/></label><label>Admin Token<input type="password" autoComplete="off" value={draft.adminToken} onChange={e=>setDraft({...draft,adminToken:e.target.value})}/></label><label>Actor ID<input value={draft.actorId} onChange={e=>setDraft({...draft,actorId:e.target.value})}/></label><div className="actions"><button onClick={apply}>应用会话配置</button><button onClick={clear}>清除</button></div><small>Token / Actor 仅存 sessionStorage。Human Decision 与 Candidate Apply 继续复用现有 Admin + Actor 机制。</small></div></aside><main><header className="topbar"><div><strong>{title}</strong><span className={`connection ${fullyConfigured?'ok':'warn'}`}>{fullyConfigured?'Admin + Actor 已配置':tokenConfigured?'Read ready · Configure Actor ID':'请配置 Admin Token / Actor'}</span></div><span>内部编辑工作台 · AI 与 Human Decision 都必须显式触发</span></header><div className="content">
  {eventId?<EventWorkbenchPage api={workbench} workflowApi={workflow} eventId={eventId} actorConfigured={actorConfigured} onBack={()=>setEventId(null)} onOpenEvent={openEvent}/>:<>
   {page==='overview'&&<EditorialOverviewPage api={workbench} onNavigate={navigate}/>} {page==='candidates'&&<EditorialCandidatesPage api={workflow} actorConfigured={actorConfigured} onOpenEvent={openEvent}/>} {page==='events'&&<EditorialEventsPage api={workbench} onOpenEvent={openEvent}/>} {page==='definitions'&&<DefinitionsPage api={adminApi}/>} {page==='instances'&&<InstancesPage api={adminApi}/>} {page==='sources'&&<SourcesPage api={adminApi}/>} {page==='schedules'&&<SchedulesPage api={adminApi}/>} {page==='runs'&&<RunsPage api={adminApi}/>} {page==='checkpoints'&&<CheckpointsPage api={adminApi}/>} {page==='risk'&&<AccountsRiskPage api={adminApi}/>} {page==='ai-providers'&&<AIProvidersPage api={adminApi}/>} {page==='ai-routes'&&<AIRoutesPage api={adminApi}/>} {page==='ai-budgets'&&<AIBudgetsPage api={adminApi}/>} {page==='ai-invocations'&&<AIInvocationsPage api={adminApi}/>} 
  </>}
 </div></main></div>
}
