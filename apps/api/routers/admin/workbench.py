...
from packages.database.models import EditorialRiskLevel, EventStatus
...
async def get_workbench_overview() -> WorkbenchOverviewResponse:
    payload = await EditorialWorkbenchQueryService().overview()
...
async def list_workbench_events(
    ...
    result = await EditorialWorkbenchQueryService().list_events(
        page=page,
        page_size=page_size,
        status=status,
        category=category,
...
async def get_workbench_event(event_id: UUID) -> WorkbenchEventDetailResponse:
    payload = await EditorialWorkbenchQueryService().event_detail(event_id)
...
async def list_workbench_event_signals(
    ...
    result = await EditorialWorkbenchQueryService().list_event_signals(
        event_id,
        page=page,
        page_size=page_size,
    )
